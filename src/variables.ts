...
export function UpdateVariables(self: WingInstance, msgs: OscMessage[]): void {
	for (const msg of msgs) {
		...
		UpdateEffectVariables(self, path, args[0]?.value as number)
	}
}
...
function UpdateEffectVariables(self: WingInstance, path: string, value: number): void {
	const match = path.match(/^\/fx\/(\d+)\/dcy$/)

	if (!match) {
		return
	}
	self.setVariableValues({ [`fx${match[1]}_decay`]: value })
}
